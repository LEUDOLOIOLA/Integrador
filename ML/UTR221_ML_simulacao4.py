...
# 4.1 OUT-OF-SAMPLE: Definir janela de teste específica (31/03/2026 08:00-11:00)
test_window_start = pd.Timestamp('2026-03-31 08:00:00')
test_window_end = pd.Timestamp('2026-03-31 11:00:00')

# Dados de treino: ANTES da janela (out-of-sample)
train_os_mask = df2.index < test_window_start
test_os_mask = (df2.index >= test_window_start) & (df2.index <= test_window_end)

X_train_os = df2.loc[train_os_mask, features]
y_train_os = df2.loc[train_os_mask, 'target']
test_data_os = df2.loc[test_os_mask]

print(f"\n--- OUT-OF-SAMPLE SPLIT ---")
print(f"Treino OOS (antes {test_window_start}): {len(X_train_os)} linhas")
print(f"Janela teste ({test_window_start} a {test_window_end}): {len(test_data_os)} linhas")

# Treinar modelo OOS
modelo_os = RandomForestRegressor(n_estimators=100, random_state=42)
modelo_os.fit(X_train_os, y_train_os)

# 4. DIVISÃO TEMPORAL (Sem shuffle!) - ORIGINAL MANTIDA
...
# ============================================================
# 11. SIMULAÇÃO OUT-OF-SAMPLE RECURSIVA (31/03/2026 08:00-11:00)
# Treinamento só com dados ANTES da janela, predição recursiva DENTRO da janela
# ============================================================

print("\n" + "=" * 70)
print("   SIMULAÇÃO OUT-OF-SAMPLE: 31/03/2026 08:00-11:00")
print("=" * 70)

# Histórico seed: últimos 60min reais ANTES da janela
pre_window = df[df.index < test_window_start].tail(60)
if len(pre_window) < 10:  # Mínimo 10 para segurança
    print("ERRO: Histórico insuficiente antes da janela de teste!")
else:
    historico_sim = list(pre_window['NIVEL-UTR-221'].values)
    
    simulacoes = []
    for idx in pd.date_range(test_window_start, test_window_end, freq='1min'):
        if idx not in df.index:
            continue
            
        row = df.loc[idx]
        # Lags do histórico simulado (feed-forward recursivo)
        nivel_lag1 = historico_sim[-1]
        nivel_lag2 = historico_sim[-2] if len(historico_sim) > 1 else nivel_lag1
        ...
        feat_values = {
            ...
            'tendencia_1min': nivel_lag1 - nivel_lag2,
            'tendencia_1h': nivel_lag1 - nivel_lag60,
            'media_movel_1h': np.mean(historico_sim[-60:]) if len(historico_sim) >= 60 else np.mean(historico_sim),
        }
        
        X_pred = pd.DataFrame([feat_values])[features]
        simulado = max(modelo_os.predict(X_pred)[0], 0)
        
        real = row['NIVEL-UTR-221']
        erro = abs(real - simulado)
        
        simulacoes.append({
            'Timestamp': idx, 
            'Real': real, 
            'Simulado': round(simulado, 4), 
            'Erro': round(erro, 4)
        })
        
        # Feed-forward: próxima iteração usa esta predição
        historico_sim.append(simulado)
    
    # Criar e salvar DF_simulacao.xlsx
    df_simulacao = pd.DataFrame(simulacoes).set_index('Timestamp')
    df_simulacao.to_excel('DF_simulacao.xlsx')
    
    # Métricas OOS
    mae_oos = mean_absolute_error(df_simulacao['Real'], df_simulacao['Simulado'])
    rmse_oos = np.sqrt(mean_squared_error(df_simulacao['Real'], df_simulacao['Simulado']))
    r2_oos = r2_score(df_simulacao['Real'], df_simulacao['Simulado'])
    mask_nz = df_simulacao['Real'] != 0
    mape_oos = np.mean(np.abs((df_simulacao.loc[mask_nz, 'Real'] - df_simulacao.loc[mask_nz, 'Simulado']) / df_simulacao.loc[mask_nz, 'Real'])) * 100 if mask_nz.sum() > 0 else 0
    
    print(f"✅ DF_simulacao.xlsx salvo: {len(df_simulacao)} minutos")
    print(f"MAE OOS:  {mae_oos:.4f} m | RMSE: {rmse_oos:.4f} m | R²: {r2_oos:.4f} | MAPE: {mape_oos:.2f}%")
    
    # Gráfico OOS
    plt.figure(figsize=(15, 8))
    plt.plot(df_simulacao.index, df_simulacao['Real'], label='Real (UTR-221)', color='blue', linewidth=2)
    plt.plot(df_simulacao.index, df_simulacao['Simulado'], label='Simulado (Recursivo OOS)', color='red', linewidth=2, linestyle='--')
    plt.fill_between(df_simulacao.index, 
                     df_simulacao['Simulado'] - df_simulacao['Erro'], 
                     df_simulacao['Simulado'] + df_simulacao['Erro'], 
                     alpha=0.2, color='orange', label='Banda de Erro')
    
    plt.title('Simulação Out-of-Sample Recursiva\n31/03/2026 08:00-11:00 (Treinado só com dados anteriores)', fontsize=14)
    plt.xlabel('Timestamp')
    plt.ylabel('Nível (metros)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('oos_simulacao_31mar.png', dpi=300, bbox_inches='tight')
    print("✅ Gráfico OOS salvo: oos_simulacao_31mar.png")
    plt.show()
    
    # Tabela resumo (primeiros e últimos 5)
    print("\nRESUMO DF_simulacao (primeiros 5):")
    print(df_simulacao.head())
    print("\nRESUMO DF_simulacao (últimos 5):")
    print(df_simulacao.tail())
    print("=" * 70)
